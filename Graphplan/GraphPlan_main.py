...
# Updated to python --version >= 3.3, by Ø. Solbø
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 1 and len(sys.argv) != 4:
        ...
        exit()
    domain = 'dwrDomain_CAMS2022.txt'   # Here you can change the Domain file
    problem = 'dwrProblem_CAMS2022.txt' # Here you can change the Problem file
    ...
    start = time.process_time()*1000 # milisecs # Updated to python --version >= 3.3, by using time.process_time() instead of time.clock()
    plan = aStarSearch(prob, heuristic)
    elapsed = time.process_time()*1000 - start # Updated to python --version >= 3.3, by using time.process_time() instead of time.clock()
    l=[]
